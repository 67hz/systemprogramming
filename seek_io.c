/**
 * seek_io.c
 * demo read(), write(), and lseek()
 *
 * Usage: seek_io file {r<length>|R<length>|w<string>|s<offset>}...
 *
 * This program opens the file name on its command line, and then performs
 * the file I/O ops specified by its remaining command-line arguments.
 *
 *      r<length>   Read 'length` bytes from the file at current
 *                  file offset, displaying them as text.
 *
 *      R<length>   Read 'length' bytes from the file at current
 *                  file offset, displaying them as hex.
 *
...
main(int argc, char *argv[])
{
    ...
    for (ap = 2; ap < argc; ap++) {
        ...
        case 'R':   /* Display bytes at current offse, in hex */
